...
RSpec.describe Session, type: :lib do
  ...
  let(:session) { Session.new email: 'testuser@google.com', password: 'password@123' }
  ...
  its(:password) { should eq 'password@123' }

  its(:decorate) { should eq subject }

  describe '#user' do
    before { expect(User).to receive(:find_by).with(email: 'testuser@google.com') }
  ...
  context 'validations' do
    ...
    context do
      ...
      before { expect(user).to receive(:authenticate).with('password@123').and_return(false) }
